...
def login():
    ...
    # 로그인 함수 호출하여 결과 확인
    login_result, user_key, user_id, user_password, user_name, user_birth, user_tel, user_use_yn, user_reg_dt, user_mod_dt = login_user(user_id, user_password)
...
def insert_data():
    ...
    val = (user_id, user_password, user_name, user_birth, user_tel, user_use_yn)
...
def update_data():
    ...
    cursor = db.cursor()
    sql = "UPDATE user_info SET user_id=%s, user_password=%s, user_name=%s, user_birth=%s, user_tel=%s, user_use_yn=%s, user_mod_dt=NOW() WHERE user_key= '" + user_key + "' "
    val = (user_id, user_password, user_name, user_birth, user_tel, user_use_yn)
...
def user_list():
    ...
    columns = ['회원 번호', '아이디', '비밀번호', '이름', '생년월일', '연락처', '유효상태', '등록일', '수정일']
